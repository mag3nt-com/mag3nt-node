...
import { Mag3ntDefaultError } from "../models/errors/mag3nt-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | Mag3ntDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | Mag3ntDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new Mag3ntDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new Mag3ntDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
